...
def test_register_election():
    u = Mock_User()
    service.register_election('any', 'Foo', 'Bar', ['a', 'b', 'c'], 2, u)
    e = u.elections[0]
    assert e.title == "Foo"
...
def test_service_add_vote(mock_get_election):
    e = Election(id=42, candidates=[Candidate(id='a'), Candidate(id='b'), Candidate(id='c')], ballot_type='any', ballots=list(), votecount=0)
...
def test_service_evaluate_election(mock_get_election):
    e = Election(id=42, committeesize=2, candidates=[Candidate(id='a'), Candidate(id='b'), Candidate(id='c')], ballot_type='any', ballots=list(), votecount=0)
